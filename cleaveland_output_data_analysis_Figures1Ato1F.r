...
T_plots_0.6_to_0.65_meta <-  T_plots_0.6_to_0.65 %>%
  ...
  ungroup() %>% 
  unique()
#write.csv(T_plots_0.6_to_0.65_meta, "T_plots_0.6_to_0.65_meta.csv")
...
#ggsave("MFEratio_distribution_0.6-0.65.png", width = 8, height = 6, bg = "white")
...
T_plots_0.6_to_0.65_sig <- T_plots_0.6_to_0.65 %>%
  ...
  slice_max(signif_count, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  inner_join(T_plots_0.6_to_0.65_subset, by = "Criteria")
#write.csv(T_plots_0.6_to_0.65_sig, "T_plots_0.6_to_0.65_sig.csv")
...
#write.csv(degradome_all_clean, file = "degradome_all_clean.csv")
...
final_list_canonical <- degradome_all_clean_canonical %>%
  ...
  slice_max(signif_count, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  inner_join(y = canonical_targets, by = "Query.and.Transcript")
...
#ggsave("canonical_targets_metrics.png", width = 8, height = 6, bg = "white")
#write.csv(final_list_canonical, "final_list_canonical.csv")
...
#write.csv(final_list_noncanonical, file = "final_list_noncanonical.csv")
...
final_list_canonical %>%
  ...
  theme_minimal(base_size = 18)
#ggsave("relationship_Allenscore_MFEratio_known.png", width = 8, height = 6, bg = "white")
...
final_list_noncanonical %>%
  ...
  theme_minimal(base_size = 18)
#ggsave("relationship_Allenscore_MFEratio_novel.png", width = 8, height = 6, bg = "white")
#write.csv(degradome_all_clean_meta, "relevant_meta_all.csv")
...
degradome_all_clean_meta <- degradome_all_clean_meta %>%
  group_by(Query.and.Transcript) %>%
  filter(AllenScore == min(AllenScore)) %>%
  filter(MFEratio == max(MFEratio)) %>%
  ungroup() %>% 
...
#write.csv(degradome_all_clean_meta, "degradome_all_clean_meta.csv")
...
final_list_canonical_subset <- final_list_canonical %>%
  select(Query.and.Transcript, MFEratio, AllenScore, signif_count)
...
T_plots_0.6_to_0.65 %>%
  ...
  bind_rows(final_list_canonical_subset) %>%
  filter(signif_count > 1) %>% 
  mutate(bin = cut(MFEratio,
  ...
  theme_classic(base_size = 18)
#ggsave("MFEratio_distribution_all.png", width = 10, height = 6, bg = "white")
...
T_plots_0.6_to_0.65 %>%
  ...
  theme_classic(base_size = 18)
#ggsave("Allenscore_distribution.png", width = 8, height = 6, bg = "white")